...
!tile spin for eigenvalues
     allocate(eps(nb*2))
!AZ 4/1/25 for closed shell 
!!     do i=1,(nB*2)
!!       eps(i) = EHFeigenup(int((i+1)/2)) 
!!       print*,'eps',i,eps(i)
!!     enddo 
!AZ 4/1/25 for open shell
     do i=1,(nb*2),2 !alpha (odd index)
      eps(i) = EHFeigenup(int((i+1)/2))
     enddo  
     do i=2,(nb*2),2 !beta (even index)
      eps(i) = EHFeigendown(int((i+1)/2))
     enddo  
!AZ 
     ...
     deallocate(tempInts1,tempInts2,tempInts3)
        ...
        !do pole=1,(neup*2)+1,2 !!!!! begin pole search, alpha
        do pole=1,(neup*2)+1,1 !!!!! begin pole search, alpha/beta

        iter=0 !max iter 15 for now

!AZ tile poles 4/1/25
        if(mod(pole,2).ne.0) then !if even pole remainder (alpha)
        print*,' '
        print*,'alpha orb',int(pole/2)+1 !pole
        print*,' '
        else
        print*,' '
        print*,'beta orb',int(pole/2) !pole
        print*,' '
        endif 
