PROGRAM uquantchem
      ...
      DOUBLE PRECISION :: ESIGMA,EDQMC,a,b,BETA,BJASTROW,CJASTROW,CUTTOFFFACTOR,MIX(2),Aexpan,LAMDA,rc,radius,GAMA,BETAA,POLY(6),deltar,TEMPERATURE,kappa,alpha,alphastart,kappastart,Egs
...
!AZ 3/30/25 
      ...
      CALL readin(CORRLEVEL,NATOMS,NLINES,ATOMICNUMBERS,APOS,Ne,MULTIPLICITY,BSURHF,Tol,WRITECICOEF,WRITEDENS,WHOMOLUMO,MESH,LIMITS,LEXCSP,NEEXC,ENEXCM,SPINCONSERVE,RESTRICT,APPROXEE, &
...
123 CONTINUE
        ...
        IF ( DKHORDER .LE. 0 ) THEN
                H0 = T+V
        ELSE
                IF ( RELAXN .OR. CFORCE .OR. MOLDYN .OR. XLBOMD ) THEN
                       WRITE(*,'(A78)')'*******************************************************************************'
                       WRITE(*,'(A78)')'***                          ABORTING CALCULATION!                          ***'
                       WRITE(*,'(A78)')'***   WARNING! Calculating scalar relatevistic forces not yet implemented.  ***'
                       WRITE(*,'(A78)')'* To use molecular dynamics, relaxing nuclear positions or calculating forces *'
                       WRITE(*,'(A78)')'*                              set DKHORDER = 0.                              *'
                       WRITE(*,'(A78)')'*******************************************************************************'
                       STOP
                ENDIF

                ALLOCATE(GVG(NB,NB))
                CALL gradpotentialgrad(BAS,NATOMS,ATOMS,GVG)
                CALL DKH(S,T,V,GVG,NB,DKHORDER,H0)
        ENDIF
        ...
        IF ( RELAXN .AND. .not. MOLDYN ) THEN
                ...
                IF ( RELALGO .EQ. 2 ) THEN
                        CALL relax(gradS,gradT,gradV,gradIntsv,S,H0,Intsv,NB,NRED,Ne,MULTIPLICITY,BSURHF,nucE,Tol,FTol,MIX,DIISORD,DIISSTART,NATOMS,NSTEPS,NLSPOINTS,PORDER,DR,BAS,ATOMS, &
                ...
                IF ( RELALGO .EQ. 1 ) THEN 
                        CALL relaxf(gradS,gradT,gradV,gradIntsv,S,H0,Intsv,NB,NRED,Ne,MULTIPLICITY,BSURHF,nucE,Tol,FTol,MIX,DIISORD,DIISSTART,NATOMS,NSTEPS,DR,BAS,ATOMS, &
                                              & APPROXEE,CORRLEVEL,PRYSR,PRYSW,PULAY,NTOTALQUAD,Q1,Q2,Q3,LORDER,CGORDER,LQ,CGQ,EETOL,&
                                              & ETEMP,EDIR,EPROFILE,EFIELDMAX,ADEF,BASAUX,ATOMSAUX,NBAUX,VRI,WRI,gradVRI,gradWRI,RIAPPROX,LIMPRECALC)
                ENDIF
                GOTO 4000
        ...
        IF ( MOLDYN ) THEN
                ...
                CALL moleculardynamics(gradS,gradT,gradV,gradIntsv,S,H0,Intsv,NB,NRED,Ne,MULTIPLICITY,BSURHF,nucE,Tol,MIX,DIISORD,DIISORDEX,DIISSTART,NATOMS,NTIMESTEPS,TIMESTEP,BAS,ATOMS,APPROXEE,&
        ...
        IF ( CORRLEVEL .EQ. 'RHF' ) THEN
                ...
                IF ( CFORCE ) THEN
                        ...
                        ELSE
                                CALL numforce(S,H0,Intsv,NB,NRED,Ne,MULTIPLICITY,BSURHF,Tol,FTol,MIX,DIISORD,DIISSTART,NATOMS,DRF,BAS,ATOMS,APPROXEE,CORRLEVEL,PRYSR,PRYSW, &
...
!AZ12/17             
                ...
                IF ( CORRLEVEL .EQ. 'EPQ3so'  ) CALL EPQ3so(MULTIPLICITY,Cup,Cdown,Ints,NB,Ne,EHFeigenup,EHFeigendown,&
                                             ETOT-nucE,nuce,SPINCONSERVE)
                IF ( CORRLEVEL .EQ. 'EPL3plusB'  ) CALL EPL3plusB(MULTIPLICITY,Cup,Cdown,Ints,NB,Ne,EHFeigenup,EHFeigendown,&
                                             ETOT-nucE,nuce,SPINCONSERVE)
                IF ( CORRLEVEL .EQ. 'EPQ3plus'  ) CALL EPQ3plus(MULTIPLICITY,Cup,Cdown,Ints,NB,Ne,EHFeigenup,EHFeigendown,&
                                             ETOT-nucE,nuce,SPINCONSERVE)
                IF ( CORRLEVEL .EQ. 'EPP3plus'  ) CALL EPP3plus(MULTIPLICITY,Cup,Cdown,Ints,NB,Ne,EHFeigenup,EHFeigendown,&
                                             ETOT-nucE,nuce,SPINCONSERVE)
