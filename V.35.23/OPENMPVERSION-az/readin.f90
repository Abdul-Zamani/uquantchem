SUBROUTINE readin(CORRLEVEL,NATOMS,NLINES,ATOMICNUMBER,SPATIALPOSITION,Ne,MULTIPLICITY,BSURHF,Tol,WRITECICOEF,WRITEDENS,WHOMOLUMO,MESH,LIMITS,LEXCSP,NEEXC,ENEXCM,SPINCONSERVE,RESTRICT,APPROXEE, & 
...
& DOTDFT,OMEGA,EDIR,NEPERIOD,EPROFILE,EFIELDMAX,ADEF,DOABSSPECTRUM,DIFFDENS,AFORCE,DRF,NSCCORR,MIXTDDFT,SCERR,RIAPPROX,LIMPRECALC,DIAGDG,FIELDDIR,FIELDREAD,DAMPING,DKHORDER,ANGtoAU,semiHF)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: NATOMS,NLINES
      CHARACTER(LEN=20), INTENT(OUT) :: CORRLEVEL,EPROFILE
      INTEGER, INTENT(OUT) :: ATOMICNUMBER(NATOMS),Ne,MULTIPLICITY,MESH(3),NEEXC,SAMPLERATE,NREPLICAS,NRECALC,NPERSIST,DIISORD,DIISSTART,NVMC,IOSA,NSTEPS,NLSPOINTS,PORDER,PULAY,FIXNSCF,DORDER,DIISORDEX,NSCCORR
      INTEGER, INTENT(OUT) :: NLEBEDEV,NCHEBGAUSS,RELALGO,ZEROSCFTYPE,EDIR,NEPERIOD,LIMPRECALC,DKHORDER
      DOUBLE PRECISION,INTENT(OUT) :: SPATIALPOSITION(NATOMS,3),Tol,LIMITS(3),ENEXCM,TIMESTEP,TEND,TSTART,BETA,BJASTROW,CJASTROW,CUTTOFFFACTOR,MIX(2),rc,DR,FTol,TEMPERATURE,kappa,alpha,EETOL,ETEMP,MIXTDDFT,SCERR
      DOUBLE PRECISION,INTENT(OUT) :: OMEGA,EFIELDMAX,DRF,FIELDDIR(3),DAMPING
      LOGICAL, INTENT(OUT) :: WRITECICOEF,WRITEDENS,WHOMOLUMO,LEXCSP,SPINCONSERVE,RESTRICT,APPROXEE,CUSPCORR,CORRALCUSP,HFORBWRITE,CFORCE,RELAXN,WRITEONFLY,MOVIE,MOLDYN,ZEROSCF,XLBOMD,RIAPPROX,BSURHF,ANGtoAU,semiHF
      ...
      IF ( finns ) THEN
        ...
        DKHORDER = 0        ! If DKHORDER > 0 then a scalar relativistic calculation will be performed
                            ! according to the Douglas-Kroll protocol. DKHORDER = Order of the DKH calculation that will be
                            ! performed. For details see J. Chem. Phys. 117, 9215 (2002)
...
!AZ
        BSURHF = .FALSE.
! Break closed-shell density matrix symmetry by setting arrays at HOMO-LUMO indices (NeDown and NeDown+1) to zero 
! in Pdown (beta density) initial guess (SCF Cycle 1).
! This option is for obtaining broken-symmetry
!open-shell singlet solutions only. 
!AZ

!AZ
!Bohr(default) to Angstrom flag      
        ANGtoAU = .FALSE. 
!Semicanon HF Fock matrix with Kohn-Sham density
        semiHF  = .FALSE. 
!AZ                               
        ...
        CUTTOFFFACTOR = 1.0d0
        MIX(1) = 0.0d0        ! MIX(1) = Miximg parameter used in the HF/DFT-calculations rho(n)*(1-MIX) + rho(n-1)*MIX -> rho(n+1)
        MIX(2) = -1.0d0       ! If MIX(2) > 0 this is a shifting parameter that is used to shift the LUMO-states of a HF/DFT
                              ! calculation in order to accelerate the scf convergence. See THeoret. Chim. Acta 46, 325-329 (1977).
                              ! IF MIX(2) < 0 then this shift is not used.
                              ! Be carefule to use with ETEMP > 0, since this might invalidate the physical meanng of this temperature
                              ! and the accompanying thermodynamical entities such as entropy, free energy and internal energy.
        ...
        DO I=1,TOTALNLINES
                ...
                ENDIF
!AZ
                IF ( VARNAME(I) .EQ. 'MULTIPLICITY') THEN
                        READ(10,*)DUMMY,MULTIPLICITY
                        LINEHASBEANREAD = .TRUE.
                ENDIF
                IF ( VARNAME(I) .EQ. 'BSURHF') THEN
                        READ(10,*)DUMMY,BSURHF
                        LINEHASBEANREAD = .TRUE.
                ENDIF
                IF ( VARNAME(I) .EQ. 'ANGtoAU') THEN
                        READ(10,*)DUMMY,ANGtoAU
                        LINEHASBEANREAD = .TRUE.
                ENDIF
                IF ( VARNAME(I) .EQ. 'semiHF') THEN
                        READ(10,*)DUMMY,semiHF
                        LINEHASBEANREAD = .TRUE.
                ENDIF
!AZ
                ...
                IF ( VARNAME(I) .EQ. 'FIELDDIR') THEN
                        ...
                        EDIR = 2
                ENDIF
                IF ( VARNAME(I) .EQ. 'DKHORDER') THEN
                        READ(10,*)DUMMY,DKHORDER
                        LINEHASBEANREAD = .TRUE.
                        FIELDREAD = .TRUE.
                ENDIF
