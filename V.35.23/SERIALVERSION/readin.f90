...
& NLSPOINTS,PORDER,WRITEONFLY,MOVIE,MOLDYN,TEMPERATURE,ZEROSCF,XLBOMD,kappa,alpha,DORDER,PULAY,FIXNSCF,NLEBEDEV,NCHEBGAUSS,EETOL,RELALGO,SOFTSTART,ETEMP)
      ...
      INTEGER, INTENT(OUT) :: ATOMICNUMBER(NATOMS),Ne,MULTIPLICITY,MESH(3),NEEXC,SAMPLERATE,NREPLICAS,NRECALC,NPERSIST,DIISORD,DIISSTART,NVMC,IOSA,NSTEPS,NLSPOINTS,PORDER,PULAY,FIXNSCF,DORDER
      ...
      LOGICAL, INTENT(OUT) :: WRITECICOEF,WRITEDENS,WHOMOLUMO,LEXCSP,SPINCONSERVE,RESTRICT,APPROXEE,CUSPCORR,CORRALCUSP,HFORBWRITE,CFORCE,RELAXN,WRITEONFLY,MOVIE,MOLDYN,ZEROSCF,XLBOMD,SOFTSTART,BSURHF
      ...
      IF ( finns ) THEN
        ...
        WRITEDENS   = .FALSE.
        WHOMOLUMO   = .FALSE.
        BSURHF = .FALSE.!AZ   ! Break closed-shell density matrix symmetry by setting arrays at HOMO-LUMO indices (NeDown and NeDown+1) to zero 
                              ! in Pdown (beta density) initial guess (SCF Cycle 1).
                        !AZ   ! This option is for obtaining broken-symmetry open-shell singlet solutions only. 
