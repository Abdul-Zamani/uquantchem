SUBROUTINE UEP2nD(MULTIPLICITY,Cup,Cdown,Ints,NB,Ne,EHFeigenup,EHFeigendown,E0,nuce,SPINCONSERVE)
      ! 
      !
 ...
 !BA
      ...
      do i=1, nb
        ...
        do j=1, nb
          ...
          do k=1, nb
            do lam=1, nb
              tempInts3(i,j,k,:) = tempInts3(i,j,k,:) + &
              (Cup(lam,k)*tempInts2(i,j,lam,:)) !AZ Cdown
            enddo
            do l=1, nb
              do sig=1, nb
  ...
  !BB
      ...
      deallocate(tempInts1,tempInts2,tempInts3)
        ...
        print*,'OSD2 is',osd2 
        print*,'Performing unrestricted non-Dyson os-D2'
...
!SE1  2ph 
        ...
        if(pole.le.nedown) then !!!!!occ IP nD
!BA       
        do i=1,Neup
          do a=Nedown+1,NB 
            do b=Neup+1,NB
               ...
               (EHFeigendown(pole) + EHFeigenup(i) -EHFeigendown(a)-EHFeigenup(b)))
            enddo
          enddo 
        enddo

        elseif(pole.gt.nedown) then !!!!!virt EA nD
 !BA        
        do i=1,Neup
          do a=Nedown+1,NB 
            do b=Neup+1,NB
        ...
        do a=Neup+1,NB
          do i=1,NeDown 
            do j=1,Neup
               ...
               (EHFeigendown(pole) + EHFeigenup(a) -EHFeigendown(i)-EHFeigenup(j)))
            enddo
          enddo 
        enddo
        endif !!!!! nD
...
!BA        
        SEold1BA = 0 !deriv of fixed number is zero
...
!BA        
       ...
       elseif(pole.gt.nedown) then !virt EA nD
!BA
       SEold2BA = 0 !deriv of fixed number is zero

       endif !! nD


       SEold1 = -1.0d0*(SEOld1BA)
       SEold2 = -1.0d0*(SEOld2BA)
